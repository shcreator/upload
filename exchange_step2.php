...
\*******************************************************************/
	...
	if (isset($_POST['currency_send']) && isset($_POST['currency_receive']) && $_POST['currency_send'] != $_POST['currency_receive'])
	{
		...
		{
			header("Location: exchange.php?currency_send=$from_id&currency_receive=$to_id&err=wrong_amount");
			exit();	
		}
	...
	if ($total > 0)
	{
		...
		{
			header("Location: exchange.php?currency_send=$from_id&currency_receive=$to_id");
			exit();
		}
		
		if ($row['min_amount'] != "" && $from_amount < $row['min_amount'])
		{
			header("Location: exchange.php?currency_send=$from_id&currency_receive=$to_id&err=min_amount");
			exit();
		}
		
		if ($row['max_amount'] != "" && $from_amount > $row['max_amount'])
		{
			header("Location: exchange.php?currency_send=$from_id&currency_receive=$to_id&err=max_amount");
			exit();
		}
		...
		{
			header("Location: exchange.php?currency_send=$from_id&currency_receive=$to_id&err=low_reserve");
			exit();		
		}
...
?>	
	...
	<div class="row">
		<div class="col-md-8">
			...
			<div class="widget">
				...
				if ($gateway == "paypal") {
	        		...
	        		<hr>
	        		<p class="text-center">
		        		<a class="btn btn-default btn-lg pull-left" href="<?php echo SITE_URL; ?>exchange.php?currency_send=<?php echo @$from_id; ?>&currency_receive=<?php echo @$to_id; ?>"><i class="fa fa-angle-left" aria-hidden="true"></i> Go Back</a>
		...
		<div class="col-md-4">
			...
			<div class="widget">
				<a href="<?php echo SITE_URL; ?>exchange.php?currency_send=<?php echo @$from_id; ?>&currency_receive=<?php echo @$to_id; ?>" class="pull-right" style="padding-top: 15px"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> change</a>
